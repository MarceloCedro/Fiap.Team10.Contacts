...
namespace Fiap.Team10.Contacts.IntegrationTests.Validations
{
    public class ContactServiceTests : BaseServiceTests
    {
        ...
        public async Task InsertContactSuccessAsync()
        {
            ...
            var exception = Record.ExceptionAsync(() => _contactService.InsertAsync(contact));
            Assert.Null(exception.Result);
        }

        [Fact]
        public async Task ContactInsertExceptionAsync()
        {            
            var contact = ContactFixtures.CreateFakeContact(0);
            var exception = await Assert.ThrowsAsync<Exception>(async () => await _contactServiceException.InsertAsync(contact));
            Assert.Equal("Some error occour when trying to insert new Contact.", exception.Message);
        }
        ...
        public async Task UpdateContactSuccessAsync()
        {
            ...
            contact.Name = "Testarindo da Silva";

            var exception = Record.ExceptionAsync(() => _contactService.UpdateAsync(contact));
            Assert.Null(exception.Result);
        }

        [Fact]
        public async Task ContactUpdateExceptionAsync()
        {
            var contact = ContactFixtures.CreateFakeContact(0);
            var exception = await Assert.ThrowsAsync<Exception>(async () => await _contactServiceException.UpdateAsync(contact));
            Assert.Equal("Some error occour when trying to update a Contact.", exception.Message);
        }
        ...
        public async Task DeleteContactSuccessAsync()
        {
            ...
            var exception = Record.ExceptionAsync(() => _contactService.DeleteAsync(contact.Id));
            Assert.Null(exception.Result);
        }

        [Fact]
        public async Task ContactDeleteExceptionAsync()
        {
            var contact = ContactFixtures.CreateFakeContact(0);
            var exception = await Assert.ThrowsAsync<Exception>(async () => await _contactServiceException.DeleteAsync(0));
            Assert.Equal("Some error occour when trying to delete a Contact.", exception.Message);
        }
    }
}
